...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 30-07-2015 a las 17:19:26
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `kardex` AS select `a`.`fecha` AS `fecha`,`b`.`modelo` AS `modelo`,`c`.`idNino` AS `idNino`,concat(`d`.`nombres`,' ',`d`.`apPaterno`,' ',`d`.`apMaterno`) AS `nombre`,`d`.`telefono` AS `telefono`,`e`.`idMateria` AS `idMateria`,`f`.`materia` AS `materia` from (((((`asistencia` `a` join `modelos` `b`) join `inscripcion` `c`) join `nino` `d`) join `horario` `e`) join `materia` `f` on(((`a`.`idModelo` = `b`.`idModelo`) and (`c`.`idInscripcion` = `a`.`idInscripcion`) and (`d`.`idNino` = `c`.`idNino`) and (`e`.`idHorario` = `c`.`idHorario`) and (`e`.`idMateria` = `f`.`idMateria`))));
